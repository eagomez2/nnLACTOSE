...
GLOBAL_FS = 44100 / 256
Coeff_B, Coeff_A = signal.butter(6, [1, 50], "bandpass", fs=GLOBAL_FS)
...
for Name in range(
    ...
    for i in range(1):
        InputPath = f"./Data/TrainData/001_{NameArray[Name]}{i+1}_bcg.csv"
        ...
        InputTemp_Filtered = signal.savgol_filter(InputTemp_Filtered, 101, 2) * -1
        InputTemp_Filtered = InputTemp_Filtered[30:-1]
        ...
        OutputTemp = OutputTemp[30:-1]
        OutputTemp = OutputTemp / np.max(np.abs(OutputTemp))

        DatasetTemp = CreateMemoryDataset(InputTemp_Filtered, OutputTemp, MemorySize=1)
...
GoodModelDirectory = "./Model2"
LayerInfoDict = {
    "0": {"size": (1, 1), "type": "input"},
    "1": {"size": 8, "type": "dense"},
    "2": {"size": 8, "type": "gru"},
    "3": {"size": 8, "type": "dense"},
    "4": {"size": 1, "type": "dense"},
}
ConditionArray = [-1, -0.6, -0.25, 0, 0.25, 0.75, 1.0]
...
Model.Train(Dataset=TrainDataset, ModelName=ModelName, Epochs=51)
...
def ModelFactory(ModelNumber):
    ...
    Sequential.add(tf.keras.layers.InputLayer(input_shape=(1,)))
    ...
    if i == 0:
        ...
        Sequential.add(tf.keras.layers.Reshape((1, 8)))
        ...
        Sequential.add(tf.keras.layers.Reshape((2, 4)))
        Sequential.add(
            tf.keras.layers.Conv1D(filters=1, kernel_size=2, activation="gelu")
        )
    ...
    Sequential.fit(TrainDataset[:, :-1], TrainDataset[:, -1], epochs=51, batch_size=484)
